...
package body Teacher_Qwen is

   function Make (Model : LLM_Qwen.Qwen_Model) return LM_Teacher is
   begin
      return (Distill.Teacher with Model => Model);
   end Make;

   function Vocab (T : LM_Teacher) return Positive is
     (Positive (LLM_Qwen.Vocab_Size (T.Model)));
   ...
   is
      ...
      Vc  : constant Integer := LLM_Qwen.Vocab_Size (T.Model);
   ...
   begin
      ...
      declare
         Flat : constant LLM_Qwen.Logits_Flat :=
           LLM_Qwen.Forward_Logits (T.Model, Ids);
      begin
         for P in 1 .. N loop
            for K in 1 .. Vc loop
