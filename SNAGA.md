...
| **LLM_Gemma** | Gemma 4 dense (E4B/12B/26B) | Q4_0, Q4_K, Q5_K | ✅ Валідація (MoE rejected) |
...
- `max_tokens` default: 2048 (override via `ASPIDA_MAX_TOKENS`)

## Команди

```bash
# Build
make server             # Зібрати E2EE inference server + client (the only chat path)
make serve              # Build + запустити server (Ctrl-C to stop)
make chat               # Зашифрований REPL до running server (SESSION=<id> to resume)

# Test (model-free)
make test               # crypto/E2EE + LLM unit tests
make test-crypto        # Тести криптографії (RFC 8439/7748/5869 vectors)
make test-llm           # LLM unit tests (incl. KV-overflow regression)
make train-test         # Training-core self-tests (grad-check, distill, QAT, BPE)
make prove              # SPARK AoRTE+functional proof (5 proven units)
make prove-flow         # SPARK flow analysis over the crypto library

# Clean
make clean              # Очистити build artifacts
```
...
---
*Auto-generated by Snaga. Last updated: 2026-06-20*
