...
int main(){
  ...
  float*pos=load("pos_add",(size_t)S*H); kadd<<<((size_t)S*H+255)/256,256>>>(hid,pos,(size_t)S*H);
  float*cosb=load("rope_cos",(size_t)S*HD),*sinb=load("rope_sin",(size_t)S*HD);
  ...
  printf("ASPIDA-STYLE CUDA ViT vs oracle: NMSE=%.3e  shape=(%d,%d)\n",se/sr,TOK,OUT);
