...
//      Matches the CPU oracle Dequant_Q8_0. Used by the prod model (Ornith-35B
...
extern "C" void aspida_gpu_chain_forward(int embed_row, int pos,
    ...
    // a hard hang on L40S (the 2026-07-13 prod first-inference hang). Set
...
#define PCH 1024 // chunk size. The Q8 matmuls use the tensor-core k_q8_wmma
                 ...
                 //
                 // PROD VRAM budget (2026-07-15): the deployed aspida-gpu is a
                 // 46GB (not 48GB) L40S that ALSO co-hosts a 3.2GB voice_server,
...
//    row = (lane>>2) + ((e & 2) ? 8 : 0)   (probe-verified on the L40S)
...
extern "C" void aspida_gpu_chain_prefill(int lane, int P, const int *rows, int pos_start,
    ...
    for (int li=0; li<NL; ++li) {
        ...
        if (!L.is_fattn) {
            ...
            //  hd=256/40k on the L40S (139 TFLOPS, ~38% of peak). fs.K/fs.V are
