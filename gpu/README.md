...
**Validated on an L40S** (batch 256, dim 512, 4 layers, 3000 steps), running
...
`libaspidatrain.so` for Ada to `dlopen` (like `LLM_GPU`). Validated on an L40S
...
runs a session from Ada. Validated end-to-end on an L40S (loss 3.6e-2 → 5.1e-15)
...
`art_w_get`/`art_w_set`). On an L40S every probe is within a combined
...
on an L40S (max rel 3.4e-3). The first transformer op toward a GPU-resident
...
difference on an L40S (max rel 5.2e-3).
...
difference on an L40S (max rel 7.9e-4).
...
difference on an L40S (max rel 2.1e-3).
...
L40S (max rel 5.3e-5). **The grad-checked kernel set for a GPU-resident Student
...
cross-entropy), grad-checked end-to-end on an L40S (max rel 2.1e-3) with a
...
transposed backward and both residual paths. Grad-checked end-to-end on an L40S
...
**Validated end-to-end on an L40S: Ada trained a full 2-layer transformer-LM on
...
L40S (max rel 1.7e-4); the resident Student shim now uses **AdamW** (bias-corrected,
...
trained two differently-sized configs end-to-end on an L40S (both loss → 0.014).
...
the FP32 path on an L40S (5.4 → 35.8 TFLOP/s, Frobenius rel err 2.6e-4). Next:
