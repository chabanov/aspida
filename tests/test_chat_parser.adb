...
--
-- Covers the four shapes an Hura (Qwen3.5) chat reply can take:
--   1) plain text only,
--   2) thinking + answer,
--   3) single tool_call,
...
begin
   ...
   -- 5) bare-form tool_call: Hura (Qwen3.5 9B) emits without angle
   ...
   -- 6) bare-form reasoning (line-start closer, real Hura shape).
   Run ("bare-think+text",
   ...
   -- 7) bare-form agentic shape (full Hura output): reasoning then
